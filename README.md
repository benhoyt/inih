...
## Compile-time options ##
  ...
  * **Allow realloc:** Have a dynamicly growing linebuffer with `-DINI_ALLOW_REALLOC=1`. Available only when using heap.
  * **Initial allocation size:** Set initial size of linebuffer with `-DINI_INITIAL_ALLOC=100`. Available only when using heap.
  * **Maximum line length:** The default maximum line length is 200 bytes. To override this, add something like `-DINI_MAX_LINE=1000`. Set to 0 to allow unlimited line length in combination with `DINI_USE_STACK=0` and `DINI_ALLOW_REALLOC=1`
