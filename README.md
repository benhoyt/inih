...
## Compile-time options ##
  ...
  * **Stack vs heap, fixed vs unlimitted line length:** By default, inih allocates its line buffer on the stack. To allocate on the heap using `malloc` instead, specify `-DINI_USE_STACK=0`, (nearly) unlimitted line length is supported using the heap.
  ...
  * **Maximum line length:** The default maximum line length is 200 bytes. To override this, add something like `-DINI_MAX_LINE=1000`. When using the heap line length is unlimitted, the value will be used for a first allocation of memory
